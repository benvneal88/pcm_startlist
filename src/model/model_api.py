...
def fuzzy_match(row, roster_df):
    ...
    first_name_match = process.extractOne(cyclist_first_name, possible_matches['cyclist_first_name'],
                                          scorer=fuzz.token_sort_ratio)
    last_name_match = process.extractOne(cyclist_last_name, possible_matches['cyclist_last_name'],
                                         scorer=fuzz.token_sort_ratio)

    if first_name_match[1] >= 80 and last_name_match[1] >= 80:
        best_match = possible_matches[(possible_matches['cyclist_first_name'] == first_name_match[0]) &
                                      (possible_matches['cyclist_last_name'] == last_name_match[0])]
...
def fuzzy_join(start_list_df, pcm_roster_df):
    ...
    logger.info(f"Performing fuzzy matching team name, first name, last name with {start_list_count} start list riders and {pcm_roster_count} pcm cyclists")
